/**
   ...
   #LIQ+#RFI+#SHIB+#DOGE+#SAFEMOON+#AUTOMATED+#FAIR+#LOCKED+#AUTOBALANCE#BANKROLL = #ONLYLAYER
   ...
   49% tokens sent to xquantum, 490T tokens (burn address with auto burn)
   ...
   50% of tokens autobalanced by xquantum to maintain volume for holders
   IMMUTABLE VALUE
...
contract XQuantum is Context, Ownable {
    ...
    function rebalance() external {
        //we should rebalance when we get more than target percentage of the supply in the xquantum
    ...
    function ready() external view returns (bool) {
        //we should rebalance when we get more than 55% of the supply in the xquantum
        uint256 upperbound = token.totalSupply().mul(upperboundPercentage).div(
            100
        );
        uint256 balance = token.balanceOf(address(this));
...
contract OnlyLayerLiquidityDrive is Context, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    constructor(uint256 _estimatedPercentageOfSupply) public {
        require(
            _estimatedPercentageOfSupply > 0 &&
                _estimatedPercentageOfSupply <= 35,
    ...
    function donate() public payable notLaunched returns (uint256) {
        require(msg.value >= 500000 ether, "Minimum donation is 500000");
        require(msg.value <= 250000000 ether, "Maximum donation is 250000000");
...
contract OnlyLayer is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public router;
    /* 
    PulseX v2 Mainnet - 0x98bf93ebf5c380C0e6Ae8e192A7e2AE08edAcc02
    */
    ...
    OnlyLayerLiquidityDrive public liquidityDrive;
    uint256 public immutable _campaignPeriod;

    XQuantum public xquantum;

    IPulseXRouter02 public immutable ipulsexV2Router;
    address public immutable ipulsexV2Pair;
    ...
    constructor(
        ...
        require(
            ...
            "Description information must be set"
        );
        require(exchangeRouter != address(0), "Exchange must be supplied");
        require(
            campaignPeriod >= 300,
            "Period must be a minimum of 300 (5 minutes)"
        );
        ...
        IPulseXRouter02 _ipulsexV2Router = IPulseXRouter02(router);
        // Create a pulsex pair for this new token
        ipulsexV2Pair = IPulseXFactory(_ipulsexV2Router.factory())
            .createPair(address(this), _ipulsexV2Router.WPLS());

        // set the rest of the contract variables
        ipulsexV2Router = _ipulsexV2Router;

        //create the liquidity drive; 25% of the supply
        liquidityDrive = new OnlyLayerLiquidityDrive(25);
        _campaignPeriod = campaignPeriod;

        //create the xquantum
        xquantum = new XQuantum();
    ...
    function launch() public returns (bool) {
        ...
        require(
            lapsed > _campaignPeriod,
            "After campaign period has ended anyone can launch the token. Try later"
        );
        ...
        uint256 _xquantumShare = _share.mul(49);
        uint256 _tokenLiquidity = _share.mul(25);
        uint256 _driveTokens = _totalTokens.sub(_share).sub(_xquantumShare).sub(
            _tokenLiquidity
        );
        ...
        //xquantum
        _transfer(address(this), address(xquantum), _xquantumShare);
    ...
    //to recieve ETH from ipulsexV2Router when swaping
    receive() external payable {}
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // also, don't swap & liquify if sender is pulsex pair.
        ...
        if (to == ipulsexV2Pair || from == ipulsexV2Pair) {
            //Buy & Sell token
            _taxFee = 5;
            _liquidityFee = 5;
            _burnFee = 0;
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the pulsex pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = ipulsexV2Router.WPLS();

        _approve(address(this), address(ipulsexV2Router), tokenAmount);

        // make the swap
        ipulsexV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        ipulsexV2Router.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function _burn(address sender, uint256 tBurn) private {
        if (tBurn > 0) {
            ...
            _rOwned[address(xquantum)] = _rOwned[address(xquantum)].add(rBurn);
            emit Transfer(sender, address(xquantum), tBurn);
        }
    }
