...
import { getNumberForMobile } from 'views/Nft/components/NftCard/HarvestAction'
    ...
    <Flex flexDirection="column">
      ...
      <>
        {isLoading || cakeAmount.isNaN() ? (
          ...
          <Balance fontSize="20px" bold unit="B XFN" value={getBalanceNumber(cakeAmount, 18)} decimals={0} />
        )}
        {isLoading || cakeAmount.isNaN() ? (
          <>
          ...
          <>
            {numberWinners !== '0' && (
              <Text fontSize="12px" color="textSubtle">
                {getFullDisplayBalance(cakeAmount.div(parseInt(numberWinners, 10)), 18, 2)}B XFN {t('each')}
              </Text>
            )}
            <Text fontSize="12px" color="textSubtle">
