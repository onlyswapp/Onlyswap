...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://onlyswap.org - check the URL carefully.")
    return warningText.split(/(https:\/\/onlyswap.org)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://onlyswap.org'}
          color={text === 'https://onlyswap.org' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
